...
public class BinFiles {
    ...
    static ByteOrder bOrder = ByteOrder.LITTLE_ENDIAN; //прямой порядок байтов
    static Map<String, String> data = new HashMap<>(); //структура данных (словарь) ключ-значение (внутри строка - строка), ключ - уникальный, если повторить ключ - то значение перезапишется
    static Charset charset = StandardCharsets.UTF_8; //указываем кодировку UTF8

    static void loadFile(String path) throws IOException { //загрузка файла
        try (InputStream stream =
                     new BufferedInputStream(new FileInputStream(path))) { //попробуй открыть поток чтения из файла по адресу path
